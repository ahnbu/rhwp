...
- PR에 포함하지 않는 로컬 전용 파일: `_docs/`, `mydocs/`, `RHWP_바로가기.lnk`, `tools/rhwp_launcher.*`, PWA/로고 자산, `CHANGELOG.md`, `CLAUDE.md`, `AGENTS.md`
...
이 프로젝트는 **하이퍼-워터폴** 방법론을 적용한다. AI 에이전트의 기본 동작(빠른 실행, 자율 수정)과 충돌이 발생할 수 있으므로 반드시 숙지한다.
...
- 작업 시간의 시작과 종료는 작업지시자가 결정한다. AI 에이전트가 임의로 작업 종료를 제안하거나 시간을 한정하지 않는다.
